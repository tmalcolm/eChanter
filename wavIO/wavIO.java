...
System.out.println(myDataSize);
	...
	public boolean save()
	{
		try
		{
			...
			FileWriter fout  = new FileWriter("waveout.h");

			byte [] bytesToConvert = new byte[2];
			for (int i=0; i<myData.length;i++) {
				bytesToConvert[0] = myData[i];
				i++;
				bytesToConvert[0] = myData[i];
				System.out.print(byteArrayToInt(bytesToConvert));
				System.out.print(", ");
				fout.write(byteArrayToInt(bytesToConvert));
				fout.write(", ");
				//fout.write(", ", 0,2);
				if (i%20 == 0) fout.write("\n");
